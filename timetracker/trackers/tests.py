...
class ActivityTestCase(TestCase):
    ...
    def setUp(self):
        ...
        TimeSpend.objects.create(activity=activity_w, date=timezone.now(), time_spent=1)
        TimeSpend.objects.create(activity=activity_w, date=timezone.now(), time_spent=1)
        TimeSpend.objects.create(activity=activity_o, date=timezone.now(), time_spent=1)
    ...
    def test_activity_time_sum(self):
        ...
        self.assertEqual(time.work_activity_time_sum(user), 2)
        self.assertEqual(time.other_activity_time_sum(user), 1)
